...
kernels = {
    "ExpSineSquared": [ExpSineSquared(l, p) for l in np.logspace(-5, 15, 10) for p in np.logspace(0, 2, 10)],
    "RBF": [RBF(length_scale) for length_scale in np.logspace(-5, 15, 10)],
    "RationalQuadratic": [RationalQuadratic(length_scale=l, alpha=a) for l in np.logspace(-2, 10, 10) for a in np.logspace(0, 2, 10)],
    "Matern": [Matern(length_scale=l, nu=nu) for l in np.logspace(-5, 15, 10) for nu in [0.5, 1.5, 2.5]],
...
print('Best estimator by GridSearchCV:', gpreg_opt.best_estimator_)
