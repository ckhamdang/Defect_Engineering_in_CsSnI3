...
lreg_opt.fit(X_train, E_form_train)
print('Best estimator by GridSearchCV:', lreg_opt.best_estimator_)

Pred_train = lreg_opt.predict(X_train)
Pred_test = lreg_opt.predict(X_test)
...
with open('Predictions_LR_CT.txt', 'w') as file:
    file.write("Dopant, Predicted Formation Energy\n")

    for i in range(len(Pred_dopant_out)):

        dopant = Pred_dopant_out[i][0]
        prediction = Pred_dopant_out[i][1]

        file.write(f'{dopant}: {prediction:.4f}\n')

print("Predictions have been saved to 'Predictions_LR_CT.txt'.")
'''

### Matrics calculation ###
"""
Prop_train_form = copy.deepcopy(E_form_train)
Pred_train_form = copy.deepcopy(Pred_train)
Prop_test_form  = copy.deepcopy(E_form_test)
Pred_test_form  = copy.deepcopy(Pred_test)
"""
